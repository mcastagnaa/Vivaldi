...
SET @RefDate = '2014/Jun/2'

-------------------------------------------------------------------
SELECT * INTO #CubeData FROM fn_GetCubeDataTable(@RefDate, 14)
-------------------------------------------------------------------
...
SELECT D.*
INTO	#GearTmpEoM
FROM	#GEARtmpData AS D INNER JOIN 
...
SET @QRY =	'SELECT DateYear * 100 + DateMonth AS DATE, ' + @COL +
			'FROM (SELECT DateYear, DateMonth, IndustryGroup, Stat FROM #LongShortDay) X
			PIVOT
...
SET @QRY =	'SELECT DateYear*100 + DateMonth AS DATE, ' + @COL +
			'FROM (SELECT DateYear, DateMonth, QuantRegion, Stat FROM #LongShortDay) X
			PIVOT
...
DROP TABLE #LongShortDay
DROP TABLE #MonthEndDates, #GearTmpEoM
DROP TABLE #GEARtmpdata
GO
