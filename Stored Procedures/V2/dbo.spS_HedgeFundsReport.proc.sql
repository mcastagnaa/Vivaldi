...
--	, RaWData.SecurityType
	...
	, RawData.BBGTicker
	, RawData.AssetCCY
	, SUM(RawData.PositionSize) AS PositionSize
	, AVG(RawData.StartPrice) AS StartPrice
	, SUM(RawData.BaseCCYExposure/NaVs.CostNaV) AS PercBaseCCYExposure
	--, NaVs.CostNaV AS NaV
	...
	, RawData.CountryISO
	, RawData.CountryName
	, ABS(SUM(RawData.PositionSize))/AVG(RawData.ADV) AS PercDayVol
	, ABS(SUM(RawData.PositionSize))/AVG(RawData.MktUnits) AS PercMarketCap
	, RawData.IsLxEM
...
WHERE	BaseData.securityGroup <> 'CashFx'
		AND BaseData.FundClass <> 'FX'
		AND BaseData.IndustrySector <> 'Equity Index'
...
SELECT	FundCode
	, IlliquidShare = SUM(CASE WHEN --ItmData.IsLxEM = 1 OR -- removed on 17/6
		ItmData.PercDayVol > 0.5
