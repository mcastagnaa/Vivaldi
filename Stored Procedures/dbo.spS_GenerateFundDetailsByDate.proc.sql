...
IF  EXISTS (
	SELECT	* 
	FROM	dbo.sysobjects 
	WHERE 	id = OBJECT_ID(N'[dbo].[spS_GenerateFundDetailsByDate]') AND 
		OBJECTPROPERTY(id,N'IsProcedure') = 1
	)
	DROP PROCEDURE [dbo].[spS_GenerateFundDetailsByDate]
GO
...
CREATE PROCEDURE [dbo].[spS_GenerateFundDetailsByDate] 
	@RefDate datetime, 
	@FundId Int,
	@PercDayVol float
AS
...
SELECT	RawData.FundId,
	...
	Beta = ISNULL(RawData.UnderBeta, RawData.Beta),
	RawData.Size,
	RawData.Value,
...
SELECT	SecurityGroup,
	SUM(PostionValue) AS PostionValue,
	FundBaseCCYCode,
	BMISCode,
	BBGTicker,
	AssetCCY,
	...
	MarketPrice,
	AssetEffect = 
		CASE WHEN Sum(PortfolioShare) <> 0 THEN
			Sum(AssetEffect*PortfolioShare)/Sum(PortfolioShare)
		ELSE 0 END,
	FxEffect = 
		CASE WHEN Sum(PortfolioShare) <> 0 THEN 
			Sum(FxEffect*PortfolioShare)/Sum(PortfolioShare)
		ELSE 0 END,
	Sum(PortfolioShare) AS PortfolioShare,
	Sum(AssetPL) AS AssetPL,
	SUM(FxPL) AS FxPL,
	SUM(PositionPL) AS PositionPL,
	SUM(BpPositionPL) AS BpPositionPL,
	AVG(MargVaRPerc) AS MargVaRPerc,
	CountryISO,
	CountryName,
	CountryRegionName,
	IndustrySector,
	...
	RiskOnPtflSh =
		CASE
			WHEN	SUM(PortfolioShare) <> 0 THEN
					ISNULL(AVG(MargVaRPerc),0)/SUM(PortfolioShare)
		ELSE
			NULL
		END,
	...
	PlOnRisk =
		CASE 
			WHEN ISNULL(AVG(MargVaRPerc),0) <>0 THEN
			ABS(SUM(PositionPl/NULLIF(TotalPl,0))/AVG(MargVarPerc))*SIGN(SUM(PositionPl))
		ELSE
			NULL
		END,
	Beta,
	...
	, ShortMom
	, UpDown 
	, SUM(AssetPlBps) AS AssetPlBps
	, SUM(FxPlBps) AS FxPlBps
	, Delta
	, Underlying
	, UnderSize
	, UnderNotional
...
GRANT EXECUTE ON spS_GenerateFundDetailsByDate TO [OMAM\StephaneD], [OMAM\MargaretA]