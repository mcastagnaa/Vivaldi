...
IF  EXISTS (
	...
	WHERE 	id = OBJECT_ID(N'[dbo].[spS_LIQ_CalcBFundsFactorsLoss]') AND 
		OBJECTPROPERTY(id,N'IsProcedure') = 1
	)
DROP PROCEDURE [dbo].[spS_LIQ_CalcBFundsFactorsLoss]
GO

CREATE PROCEDURE [dbo].[spS_LIQ_CalcBFundsFactorsLoss] 
	@RefDate datetime
	, @FundId int
...
SELECT 	@RefDate AS RefDate
	...
	, Positions.BMISCode 
	, Positions.BBGTicker
	, ABS(Positions.ExpWeight) AS ExpWeight
...
SELECT 	Factors.RefDate
	, Factors.FundId
	, SUM(Factors.ExpWeight * Factors.AgeOfBond) AS AgeOfBond
	, SUM(Factors.ExpWeight * Factors.BidAsk) AS BidAsk
	, SUM(Factors.ExpWeight * Factors.TimeToMaturity) AS TimeToMaturity
	, SUM(Factors.ExpWeight * Factors.IssuedAmount) AS IssuedAmount
	, SUM(Factors.ExpWeight * Factors.BadPrice) AS BadPrice
	, SUM(Factors.ExpWeight * Factors.YieldVol) AS YieldVol
	, SUM(Factors.ExpWeight * Factors.PositionSize) AS PositionSize
	, Factors.FundCode
	, NaVs.CostNaV
...
----------------------------------------------------------------------------------
GRANT EXECUTE ON dbo.spS_LIQ_CalcBFundsFactorsLoss TO [OMAM\StephaneD]