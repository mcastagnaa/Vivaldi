...
IF  EXISTS (
	...
	WHERE 	id = OBJECT_ID(N'[dbo].[spS_FundFactorsRisk]') AND 
		OBJECTPROPERTY(id,N'IsProcedure') = 1
	)
DROP PROCEDURE [dbo].[spS_FundFactorsRisk]
GO

CREATE PROCEDURE [dbo].[spS_FundFactorsRisk] 
	@RefDate datetime
	, @FundId int 		-- 20
	, @LiqPerc float	-- 0.1
AS
...
SELECT	*
INTO #SMBExp
FROM (	SELECT	MargVarPerc, LongShort, Size 
	FROM #RawData
	WHERE len(Size)>0 ) o
PIVOT (SUM(MargVarPerc) FOR LongShort IN(	[Long]
					, [Short]
					)) p

SELECT	*
INTO #HMLExp
FROM (	SELECT	MargVarPerc, LongShort, Value 
	FROM #RawData
	WHERE len(Value)>0) o
PIVOT (SUM(MargVarPerc) FOR LongShort IN(	[Long]
					, [Short]
...
SELECT	*
INTO #UMDExp
FROM (	SELECT	MargVarPerc, LongShort, UpDown 
	FROM #RawData
...
SELECT	*
INTO #BetaExp
FROM (	SELECT	MargVarPerc, LongShort, 
...
GRANT EXECUTE ON spS_FundFactorsRisk TO [OMAM\StephaneD], [OMAM\MargaretA]