...
IF  EXISTS (
	...
	WHERE 	id = OBJECT_ID(N'[dbo].[spS_GetFoFTypeLoading]') AND 
		OBJECTPROPERTY(id,N'IsProcedure') = 1
	)
DROP PROCEDURE [dbo].[spS_GetFoFTypeLoading]
GO

CREATE PROCEDURE [dbo].[spS_GetFoFTypeLoading] 
	@RefDate datetime,
	@FundId int 
AS
...
WHERE	FundId = @FundId
	AND DetsDate > DATEADD(month,-3,@RefDate)
GROUP BY	DetsDate
		, SecurityGroup
		, IndustrySector
		, IndustryGroup
...
GRANT EXECUTE ON spS_GetFoFTypeLoading TO [OMAM\StephaneD]