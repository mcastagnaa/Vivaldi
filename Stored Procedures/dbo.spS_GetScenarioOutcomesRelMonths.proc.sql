...
IF  EXISTS (
	...
	WHERE 	id = OBJECT_ID(N'[dbo].[spS_GetScenarioOutcomesRelMonths]') AND 
		OBJECTPROPERTY(id,N'IsProcedure') = 1
	)
DROP PROCEDURE [dbo].[spS_GetScenarioOutcomesRelMonths]
GO

CREATE PROCEDURE [dbo].[spS_GetScenarioOutcomesRelMonths] 
	@RefDate datetime,
	@FundId int,
	@Months int 
AS
...
WHERE	Scen.FundId = @FundId
	AND Scen.ReportDate > DATEADD(month,-@Months, @RefDate)
...
GRANT EXECUTE ON spS_GetScenarioOutcomesRelMonths TO [OMAM\StephaneD]