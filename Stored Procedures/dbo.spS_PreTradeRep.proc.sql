...
IF  EXISTS (
	...
	WHERE 	id = OBJECT_ID(N'[dbo].[spS_PreTradeRep]') AND 
		OBJECTPROPERTY(id,N'IsProcedure') = 1
	)
DROP PROCEDURE [dbo].[spS_PreTradeRep]
GO

CREATE PROCEDURE [dbo].[spS_PreTradeRep] 
	@RefDate datetime
	, @AbsRepId int
	, @RelRepId int
AS
...
SELECT	ReportDate
	, 2 * VarBench/NaVs.CostNaV AS TwoBenchVaR
	, [VaR]/NaVs.CostNaV AS PortfVaR
INTO	#LastBenchVaR
...
GRANT EXECUTE ON spS_PreTradeRep TO [OMAM\StephaneD], [OMAM\MargaretA] 