...
IF  EXISTS (
	...
	WHERE 	id = OBJECT_ID(N'dbo.spS_GetLastProdChanges') AND 
		OBJECTPROPERTY(id,N'IsProcedure') = 1
	)
DROP PROCEDURE dbo.spS_GetLastProdChanges
GO

CREATE PROCEDURE dbo.spS_GetLastProdChanges
	@RefDate datetime
	, @Lookback integer
AS
...
WHERE	C.ChangeDate between DATEADD(mm,-@lookback, @RefDate) AND @RefDate
ORDER BY E.Id, C.ChangeDate DESC

-----------------------------------------------------------------

GO
GRANT EXECUTE ON dbo.spS_GetLastProdChanges TO [OMAM\Compliance]