...
SELECT	RawData.SecurityType,
	...
	RawData.AssetCCY,
	(CASE 
		WHEN RawData.SecurityType in ('Bonds', 'Cash', 'CFD', 'Equities', 'Futures Offset', 'FX', 'Others', 'Placing',
			'MMFunds', 'TBills', 'CD', 'IndexFut', 'BondFut', 'IntRateFut') THEN
		dbo.fn_GetBaseCCYPrice(
			(CASE WHEN RawData.SecurityType = 'IntRateFut'
