...
IF  EXISTS (
	...
	WHERE 	id = OBJECT_ID(N'[dbo].[spS_FundFactorsPl]') AND 
		OBJECTPROPERTY(id,N'IsProcedure') = 1
	)
DROP PROCEDURE [dbo].[spS_FundFactorsPl]
GO

CREATE PROCEDURE [dbo].[spS_FundFactorsPl] 
	@RefDate datetime
	, @FundId int 		-- 20
	, @LiqPerc float	-- 0.1
AS
...
SELECT	*
INTO #SMBExp
FROM (	SELECT	BpPositionPL, LongShort, Size 
	FROM #RawData
	WHERE len(Size)>0 ) o
PIVOT (SUM(BpPositionPL) FOR LongShort IN(	[Long]
					, [Short]
					)) p

SELECT	*
INTO #HMLExp
FROM (	SELECT	BpPositionPL, LongShort, Value 
	FROM #RawData
	WHERE len(Value)>0) o
PIVOT (SUM(BpPositionPL) FOR LongShort IN(	[Long]
					, [Short]
...
SELECT	*
INTO #UMDExp
FROM (	SELECT	BpPositionPL, LongShort, UpDown 
	FROM #RawData
...
SELECT	*
INTO #BetaExp
FROM (	SELECT	BpPositionPL, LongShort, 
...
GRANT EXECUTE ON spS_FundFactorsPl TO [OMAM\StephaneD], [OMAM\MargaretA]